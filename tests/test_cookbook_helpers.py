...
def test_serve_runner_preserves_command_exit_code():
    ...
    assert "AUSTIN_CMD_EXIT=$?" in script
    assert 'echo "=== Process exited with code $AUSTIN_CMD_EXIT ==="' in script
...
def test_pip_serve_runner_emits_download_ok_before_exit_marker():
    ...
    assert 'exit "$AUSTIN_CMD_EXIT"' in script
...
def test_llama_cpp_linux_bootstrap_checks_cudart_before_cuda_build():
    ...
    assert '_austin_has_cudart' in script
    ...
    assert script.index('_austin_has_cudart') < script.index('DGGML_CUDA=ON')
