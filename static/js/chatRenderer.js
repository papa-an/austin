...
const _ACTION_RECENTS_KEY = 'austin-msg-actions-recent';
 ...
 */
const _USER_ACTION_RECENTS_KEY = 'austin-user-actions-recent';
    ...
    ctxRing.addEventListener('click', (e) => {
      ...
      if (compactBtn) {
        compactBtn.addEventListener('click', async (e) => {
          ...
          compactRole.textContent = 'Austin';
...
export function addMessage(role, content, modelName, metadata) {
  try {
    ...
    var _roleText = role === 'user' ? 'You' : (isSlash || isCompacted) ? 'Austin' : shortModel(resolvedModel);
