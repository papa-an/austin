#!/bin/bash
# Build a downloadable macOS launcher app + .dmg for Austin.
#
#   ./build-macos-app.sh
#
# Produces:
#   dist/Austin.app   — double-click: starts the local server (using this
#                         repo's venv) and opens the UI in an app-style window.
#   dist/Austin.dmg   — drag-to-Applications disk image (the downloadable).
#
...
# so rebuild if you move the repo. Override the port with AUSTIN_PORT.
set -e

REPO_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
APP_NAME="Austin"
INSTALL_DIR="$REPO_DIR"
PORT="${AUSTIN_PORT:-7860}"
DIST="$REPO_DIR/dist"
...
if [ -f "$REPO_DIR/docs/odysseus.jpg" ] && command -v sips >/dev/null 2>&1; then
  ...
  if sips -s format icns "$TMPIMG/icon.png" --out "$APP/Contents/Resources/austin.icns" >/dev/null 2>&1; then
    echo "  icon:        austin.icns"
  else
...
<dict>
    ...
    <key>CFBundleIdentifier</key>      <string>com.austin.launcher</string>
    <key>CFBundleVersion</key>         <string>1.0</string>
    <key>CFBundleShortVersionString</key><string>1.0</string>
    <key>CFBundlePackageType</key>     <string>APPL</string>
    <key>CFBundleExecutable</key>      <string>$APP_NAME</string>
    <key>CFBundleIconFile</key>        <string>austin</string>
...
#!/bin/bash
# Austin.app — start the local server and open the UI in an app window.
...
LOG="$INSTALL_DIR/logs/austin-app.log"

notify() { /usr/bin/osascript -e "display notification \"$1\" with title \"Austin\"" >/dev/null 2>&1; }
die_gui() {
  /usr/bin/osascript -e "display dialog \"$1\" with title \"Austin\" buttons {\"OK\"} default button 1 with icon stop" >/dev/null 2>&1
  exit 1
}

[ -x "$UVICORN" ] || die_gui "Austin isn't set up yet. Open Terminal and run:
...
for i in $(seq 1 120); do
  ...
  kill -0 "$SERVER_PID" 2>/dev/null || die_gui "Austin failed to start. Log:
$LOG"
  sleep 1
done
...
if [ "$READY" = "1" ]; then
  ...
  notify "Austin is taking a while — open $URL once it finishes starting."
fi
wait "$SERVER_PID"
LAUNCHER
...
echo "Install:       open '$DIST/$APP_NAME.dmg'  (drag Austin to Applications)"
