...
On first setup, AUSTIN creates an admin account (`admin` unless
`AUSTIN_ADMIN_USER` is set) and prints a temporary password in the terminal.
...
M-series Mac, run AUSTIN natively:
...
```bash
AUSTIN_HOST=0.0.0.0 ./start-macos.sh
...
ntfy. AUSTIN and the bundled service ports bind to `127.0.0.1` by default, so
...
AUSTIN SSH key and add the public key to the remote server's
...
does not mount `/var/run/docker.sock`. You can still connect AUSTIN to
...
On Arch Linux, verify the host NVIDIA driver and Docker GPU passthrough before enabling the AUSTIN NVIDIA overlay.
...
Then enable the AUSTIN NVIDIA compose overlay:

```env
COMPOSE_FILE=docker-compose.yml:docker/gpu.nvidia.yml
```

Rebuild and verify the GPU inside the AUSTIN container:
...
This connects AUSTIN in Docker to an Ollama server that is already running on
your host machine; it does not start Ollama inside the container.
`host.docker.internal` is Docker's hostname for the host machine from inside the
container. Cookbook **Serve** is a separate workflow for serving downloaded
models through AUSTIN/llama.cpp, so Windows users with an existing Ollama
...
do not run on macOS. MLX-only models are not served by AUSTIN.
...
does **not** read `APP_BIND` / `AUSTIN_HOST` from `.env`, so editing `.env`
...
[Ollama](https://ollama.com/download) is the easiest path — point AUSTIN at
...
If `chromadb-client` (the lightweight HTTP-only package) is installed alongside the full `chromadb` package, AUSTIN starts but ChromaDB silently falls back to HTTP-only mode and fails.
...
To expose AUSTIN on a local network or Tailscale with HTTPS:
1. Change the bind address to `0.0.0.0` in `.env` (`APP_BIND=0.0.0.0` or `AUSTIN_HOST=0.0.0.0`).
...
3. Run `uvicorn` with the generated certs:
   ...
   ```
4. Install the `mkcert` CA on any other device you want to access AUSTIN from (e.g., for iOS, email the `rootCA.pem` to yourself, install the profile, and trust it in Certificate Trust Settings).

### Common self-host traps (30-second fixes)
A grab-bag of small gotchas that otherwise turn into long debugging sessions.

- **`AUTH_ENABLED=false` is ignored / you're still forced to log in (Windows).** If you edited `.env` in Notepad it may have saved a UTF-8 **BOM**, turning the first key into `﻿AUTH_ENABLED` so it is never matched. AUSTIN loads `.env` with `encoding="utf-8-sig"` to tolerate a leading BOM, but the safe fix is to re-save `.env` as **UTF-8 without BOM** (VS Code: *Save with Encoding → UTF-8*).
...
- **Calendar/contacts (Radicale) won't sync.** Point AUSTIN at the **full collection URL** with its trailing slash — e.g. `http://host:5232/<user>/<collection-id>/` — not just the server root. Radicale shows this address for each calendar/address book in its web UI.
...
| `kokoro`, `soundfile` | Local Kokoro-82M text-to-speech on a CUDA GPU. The pinned Kokoro release supports AUSTIN installs on Python 3.11-3.12; these packages are intentionally skipped on Python 3.13+ (including the Python 3.14 container image). |
...
The default Docker image currently uses Python 3.14, while Kokoro 0.9.4 declares Python `>=3.10,<3.13`. AUSTIN itself continues to support Python 3.11+, but this pinned optional local-TTS feature requires a native Python 3.11 or 3.12 environment. Kokoro declares `torch`, but the local provider only activates when that torch build has CUDA and a GPU is visible; install the CUDA build appropriate for your host. Browser and configured endpoint TTS remain available on Python 3.13+ and in the container image.
...
AUSTIN email accounts currently use IMAP/SMTP username-password auth. Outlook
and Microsoft 365 generally require OAuth instead, so normal Microsoft mailbox
passwords will fail. See [docs/email-outlook.md](docs/email-outlook.md) for the
current limitation and the planned integration direction.

## Security Notes
AUSTIN is a self-hosted workspace with powerful local tools: shell access, file uploads, model downloads, web research, email/calendar integrations, and API tokens. Treat it like an admin console.

- Keep `AUTH_ENABLED=true` for any network-accessible deployment.
- Keep `LOCALHOST_BYPASS=false` outside local development.
- Leave `SECURE_COOKIES` unset unless you need to override it: session cookies are marked `Secure` whenever the request arrives over HTTPS. Use `SECURE_COOKIES=true` to force it on for a proxy whose scheme AUSTIN cannot see, or `SECURE_COOKIES=false` to force it off while you still serve plain HTTP alongside HTTPS.
...
- Keep ChromaDB, SearXNG, ntfy, Ollama, vLLM, llama.cpp, databases, and raw model/provider APIs internal-only. Expose only the authenticated AUSTIN web/API entrypoint through your trusted proxy or private access layer.
...
AUSTIN serves plain HTTP on its app port. Docker Compose binds AUSTIN and the bundled services to `127.0.0.1` by default, so a typical production/private setup is:

1. Keep AUSTIN on localhost, for example `127.0.0.1:7000`.
2. Terminate HTTPS at a trusted reverse proxy or private access gateway.
3. Put the authenticated AUSTIN web/API entrypoint behind that layer.
4. Keep raw service and model ports internal-only.

Cloudflare Access, Tailscale, Caddy, nginx, and Traefik can all fit this pattern; none are required by AUSTIN. If your access layer reaches AUSTIN on the same host, proxy to `http://127.0.0.1:7000` and keep `AUTH_ENABLED=true` and `LOCALHOST_BYPASS=false`. Any proxy that forwards `X-Forwarded-Proto: https` gets `Secure` session cookies without configuration, so `SECURE_COOKIES` only needs setting when you want to override that — force it on for a proxy that forwards no scheme at all, or off while you still serve plain HTTP.
...
AUSTIN needs no changes for this — uvicorn keeps speaking HTTP/1.1 on
...
machine. Replace `7000` if AUSTIN listens elsewhere — the macOS start script
uses `7860`.
...
AUSTIN's own service is unchanged; the proxy runs alongside it. Under Docker,
...
A proxy that exposes the HTTPS request scheme to AUSTIN needs no `SECURE_COOKIES` setting. Only force it on when the proxy cannot expose that scheme:
...
- Leave `SECURE_COOKIES` unset when AUSTIN can see the external HTTPS scheme;
...
- AUSTIN sends `Strict-Transport-Security` once it sees `X-Forwarded-Proto:
  ...
  have visited AUSTIN. Give AUSTIN its own hostname, or strip the header at
...
| `AUSTIN_CHAT_UPLOAD_MAX_BYTES` | `10485760` | Chat/agent attachment cap in bytes. Raise for larger local PDFs or text documents. |
| `AUSTIN_GALLERY_UPLOAD_MAX_BYTES` | `104857600` | Gallery image upload cap in bytes (100 MB). |
| `AUSTIN_GALLERY_TRANSFORM_UPLOAD_MAX_BYTES` | `26214400` | Gallery transform input cap in bytes (25 MB). |
| `AUSTIN_MEMORY_IMPORT_MAX_BYTES` | `10485760` | Memory import file cap in bytes (10 MB). |
| `AUSTIN_PERSONAL_UPLOAD_MAX_BYTES` | `26214400` | Personal document upload cap in bytes (25 MB). |
| `AUSTIN_EMAIL_COMPOSE_UPLOAD_MAX_BYTES` | `26214400` | Email compose attachment cap in bytes (25 MB). |
| `AUSTIN_STT_MAX_AUDIO_BYTES` | `26214400` | Speech-to-text audio cap in bytes (25 MB). |
| `AUSTIN_ICS_MAX_BYTES` | `10485760` | Calendar `.ics` import cap in bytes (10 MB). |

All upload-limit vars are validated (must be a positive integer) and optional; an invalid value fails fast at startup.

### Built-in MCP servers (optional setup)

AUSTIN auto-registers a few built-in MCP servers at startup. The npx-based ones (currently the browser server, `@playwright/mcp`) only start when their npm package is already in the local npx cache. If a package isn't cached, that server is skipped with a startup log message explaining what to do, so a fresh install does not block on a multi-minute npm download or hang if Playwright system deps are missing.
...
That installs `@playwright/mcp` plus Playwright (~300MB total). Restart AUSTIN and the server will register at startup.
